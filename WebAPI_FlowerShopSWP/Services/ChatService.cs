...
namespace WebAPI_FlowerShopSWP.Services
{
    public class ChatService : IChatService
    {
        ...
        private async Task SendMessageNotification(Message message, int receiverId, Conversation conversation)
        {
            try
            {
                // Kiểm tra tin nhắn cuối cùng đã thông báo
                    ...
                    || (DateTime.UtcNow - lastNotifiedMessage.CreatedAt).TotalMinutes > 5)
                {
                    ...
                    string notificationContent = $"Tin nhắn mới từ {role} {message.Sender.Name}";
            ...
            {
                // Log lỗi nhưng không throw exception để không ảnh hưởng đến việc gửi tin nhắn
            }
        }
        public async Task<MessageResponseDto> SendMessage(int senderId, SendMessageDto dto)
        {
            ...
            try
            {
                ...
                if (dto.Image != null)
                {
                    ...
                    _logger.LogInformation($"Image saved with URL: {imageUrl}"); // Thêm log
                }
                ...
                var message = new Message
                {
                    ...
                    ImageUrl = imageUrl, // Đảm bảo imageUrl được gán
                ...
                // Log message details
                _logger.LogInformation($"Message saved: ID={message.MessageId}, " +
        ...
        private ConversationResponseDto MapToConversationResponse(Conversation conversation)
        {
            ...
            return new ConversationResponseDto
            {
                ...
                Seller = new UserInfoDto
                {
                    ...
                    Name = conversation.Seller?.Name ?? "Unknown",
